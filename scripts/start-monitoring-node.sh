...
bash "${SCRIPT_DIR}/start-monitoring-node.sh" "${APP_HOST}"
EOF
      exit $?
    fi

    log_fatal "Docker is still not reachable after attempting group refresh. Reconnect SSH so Linux reloads docker group membership, then rerun ./scripts/start-monitoring-node.sh ${APP_HOST}."
  fi
